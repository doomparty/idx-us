...
# 基础配置
BASE_PREFIX = "9000-firebase-lost-"
...
def find_9000_firebase_lost_jwt_and_domain(cookie_path="cookie.json"):
    """
    直接遍历cookie，找到domain以9000-firebase-lost-开头的WorkstationJwtPartitioned，返回其domain和JWT
    """
    try:
        ...
        for cookie in data.get("cookies", []):
            if (
                ...
                and cookie.get("domain", "").startswith("9000-firebase-lost-")
            ):
                return f"https://{cookie['domain']}", cookie["value"]
    except Exception as e:
        log_message(f"查找9000-firebase-lost域名和JWT时出错: {e}")
    return None, None
...
def send_to_telegram(message):
    ...
    # 添加工作站域名信息(如果存在)
    domain, _ = find_9000_firebase_lost_jwt_and_domain(cookies_path)
    if not domain:
...
def check_page_status_with_requests():
    ...
    try:
        ...
        preset_url = 'https://9000-firebase-lost-1747192543356.cluster-f4iwdviaqvc2ct6pgytzw4xqy4.cloudworkstations.dev'

        # 优先查找cookie.json中匹配的9000-firebase-lost域名和JWT
        domain, jwt = find_9000_firebase_lost_jwt_and_domain(cookies_path)
        if domain and jwt:
            preset_url = domain
            preset_jwt = jwt
            log_message("已优先使用cookie.json中匹配的9000-firebase-lost域名和JWT")
        else:
            # 兼容原有逻辑
            try:
    ...
    try:
        ...
        if len(parts) >= 2:
            ...
            if 'aud' in payload:
                ...
                match = re.search(r'(9000-firebase-lost-[^\.]+\.cluster-[^\.]+\.cloudworkstations\.dev)', aud)
                if match:
                    domain_suffix = match.group(1).split('9000-firebase-lost-')[1]
